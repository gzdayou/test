                ...
                if( date_type == 1 ) {
                    for( i=0; i<24; i++ ) {
                        if( typeof data[i] != 'undefined' ) {
                            t1.push(parseFloat(data[i].T1));
                            t2.push(parseFloat(data[i].T2));
                        } else {
                            t1.push(0);
                            t2.push(0);
                        }
                    }
                    chart_option.series[0].data = t1;
                    chart_option.series[1].data = t2;
                    $('#chart_content').highcharts(chart_option);
                }
                //按月折线图
                if( date_type == 2 ) {
                    ...
                    for( j=1; j<=t.days; j++ ) {
                        ...
                        if( typeof data[j] != 'undefined' ) {
                            t1.push(parseFloat(data[j].T1));
                            t2.push(parseFloat(data[j].T2));
                        } else {
                            t1.push(0);
                            t2.push(0);
                        }
