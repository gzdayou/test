            ...
            <div class="mod_setting">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" value="48" name="FREQ_RSBEMER" id="FREQ_RSBEMER" data-curmod="" class="input-txt" onmouseout="this.style.backgroundColor='transparent';this.style.borderColor='#508c8c';">
                            Hz
                        </td>
                    </tr>
                    <tr>
        ...
        $.ajax({
            ...
            success: function (t) {//console.log(t)
                if( t.status == 1 ) {
                    ...
                    var RSBInfo = t.data.MSGOBJ.RSBState.RSBInfo[idx][0] ;
                    //console.log(RSBInfo);

                    //当日开机时间
                    var starttime = RSBInfo.StartTime;
                    var newDate = new Date();
                    newDate.setTime(starttime * 1000);
                    starttime = newDate.format('h:m:s');
                    //当日关机时间
                    var endtime = RSBInfo.StopTime;
                    newDate.setTime(endtime * 1000);
                    endtime = newDate.format('h:m:s');

                    $("#CurPower").html(RSBInfo.CurPower > 0 ? RSBInfo.CurPower.toFixed(1) : 0);
                    $("#CurFreq").html(RSBInfo.CurFreq > 0 ? RSBInfo.CurFreq.toFixed(1) : 0);
                    $("#CurEnergy").html(RSBInfo.CurEnergy.toFixed(1));
                    $("#CurTime").html(RSBInfo.CurTime);
                    $("#TotalEnergy").html(RSBInfo.TotalEnergy.toFixed(0));
                    $("#TotalTime").html(RSBInfo.TotalTime);
                    $("#StartTime").html(dateFtt("hh:mm:ss",RSBInfo.StartTime));
                    $("#StopTime").html(dateFtt("hh:mm:ss",RSBInfo.StopTime));

                    //运行状态
                    if( RSBInfo.CurState & 1 ) {
                    ...
                    var value_c_one = RSBInfo.CurEleA; //能力值取代码置于值于此处
                    ...
                    var value_c_two = RSBInfo.CurEleB; //能力值取代码置于值于此处
                    ...
                    var value_c_three = RSBInfo.CurEleC; //能力值取代码置于值于此处
                    ...
                    var value_t_one = RSBInfo.CurVolA; //能力值取代码置于值于此处
                    ...
                    var value_t_two = RSBInfo.CurVolB; //能力值取代码置于值于此处
                    ...
                    var value_t_three = RSBInfo.CurVolC; //能力值取代码置于值于此处
        ...
        $.ajax({
            ...
            success: function (t) {//console.log(t)
                if( t.status == 1 ) {
                    ...
                    $("#FREQ_RSBEMER").attr("data-curmod",SysInfo.ControlMode);
                    
                    var ldb_mod = (SysInfo.ControlMode & 4) ? 1 : 0;
...
function turn( flag ) {
    ...
    var commond = "SWITCH_RSB" + idx;

    var ctl = (flag == 1) ? "运行" : "停止";

    layer.alert('确认'+ ctl +'热水泵'+idx+'吗？', 
        {
    ...
    layer.alert("确认保存控制模式设置吗？", 
        {
            ...
            yes:function(index){
                ...
                if( $("#FREQ_RSBEMER").val() != '' && !$("#onoffswitch_rsb").prop('checked') ) {
                    $.ajax({
                        url: "<?php echo ADMIN_SITE_URL;?>/modules/system/index.php?act=device_control&op=ctrlcmd",
                        type: "get",
                        timeout : 500,
                        dataType: "json",
                        data: { msg_name: 'FREQ_RSBEMER', msg_value : $("#FREQ_RSBEMER").val() },
                        success: function (t) {
                            if( t.status == '1' ) {
                                //alert("操作成功");
                            } else {
                ...
                //控制模式
                var c_model = $("#FREQ_RSBEMER").attr("data-curmod");
                //console.log(c_model);
                c_model = c_model & 3 ;
                //console.log(c_model);
