            ...
            <div class="mod_setting">
                ...
                <table>
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="text" value="48" name="FREQ_LDBEMER" id="FREQ_LDBEMER" data-curmod="" class="input-txt" onmouseout="this.style.backgroundColor='transparent';this.style.borderColor='#508c8c';">
                            Hz
                        </td>
                    </tr>
                    <tr>
...
$(function(){
    ...
    function ajaxFunc() {
        $.ajax({
            ...
            timeout : 500,
            dataType: "json",
            data: {msg_name: "LDBState"},
            success: function (t) {//console.log(t)
                if( t.status == 1 ) {
                    var idx = <?php echo $output['idx']; ?>;
                    
                    var LDBInfo = t.data.MSGOBJ.LDBState.LDBInfo[idx][0] ;
                    //console.log(LDBInfo);

                    $("#CurPower").html(LDBInfo.CurPower > 0 ? LDBInfo.CurPower.toFixed(1) : 0);
                    $("#CurFreq").html(LDBInfo.CurFreq > 0 ? LDBInfo.CurFreq.toFixed(1) : 0);
                    $("#CurEnergy").html(LDBInfo.CurEnergy.toFixed(1));
                    $("#CurTime").html(LDBInfo.CurTime);
                    $("#TotalEnergy").html(LDBInfo.TotalEnergy.toFixed(0));
                    $("#TotalTime").html(LDBInfo.TotalTime);
                    $("#StartTime").html(dateFtt("hh:mm:ss",LDBInfo.StartTime));
                    $("#StopTime").html(dateFtt("hh:mm:ss",LDBInfo.StopTime));
                    
                    //运行状态
                    if( LDBInfo.CurState & 1 ) {
                    ...
                    //非控状态
                    if( LDBInfo.CurMode != 1 ) {
                        $("#sb_status").html('<img src="<?php echo ADMIN_RESOURCE_URL;?>/image/SB_NC.png" >');
                    } else {
						$("#sb_status").html('');
					}
                    //模式显示
					$("#zc").html('<img src="<?php echo ADMIN_RESOURCE_URL;?>/image/btn_status_zc_normal.png" >');
					$("#jd").html('<img src="<?php echo ADMIN_RESOURCE_URL;?>/image/btn_status_jd_normal.png" >');
					$("#pl").html('<img src="<?php echo ADMIN_RESOURCE_URL;?>/image/btn_status_pl_normal.png" >');
                    //正常
                    if( LDBInfo.CurMode == 1 ) {
                        $("#zc").html('<img src="<?php echo ADMIN_RESOURCE_URL;?>/image/btn_status_zc_error.png" >');
                    }
                    //就地
                    if( LDBInfo.CurMode == 2 ) {
                        $("#jd").html('<img src="<?php echo ADMIN_RESOURCE_URL;?>/image/btn_status_jd_error.png" >');
                    }
                    //旁路
                    if( LDBInfo.CurMode == 4 ) {
                        $("#pl").html('<img src="<?php echo ADMIN_RESOURCE_URL;?>/image/btn_status_pl_error.png" >');
                    }
					//故障
                    if( LDBInfo.CurState & 2 ) {
                    ...
                    var value_c_one = LDBInfo.CurEleA; //能力值取代码置于值于此处
                    ...
                    var value_c_two = LDBInfo.CurEleB; //能力值取代码置于值于此处
                    ...
                    var value_c_three = LDBInfo.CurEleC; //能力值取代码置于值于此处
                    ...
                    var value_t_one = LDBInfo.CurVolA; //能力值取代码置于值于此处
                    ...
                    var value_t_two = LDBInfo.CurVolB; //能力值取代码置于值于此处
                    ...
                    var value_t_three = LDBInfo.CurVolC; //能力值取代码置于值于此处
        ...
        $.ajax({
            ...
            success: function (t) {//console.log(t)
                if( t.status == 1 ) {
                    ...
                    $("#FREQ_LDBEMER").attr("data-curmod",SysInfo.ControlMode);
...
function turn( flag ) {
    ...
    var commond = "SWITCH_LDB" + idx;

    var ctl = (flag == 1) ? "运行" : "停止";
    var msg = '确认'+ ctl +'冷冻水泵'+idx+'吗？';
    ...
    layer.alert("确认保存控制模式设置吗？", 
        {
            ...
            yes:function(index){
                ...
                if( $("#FREQ_LDBEMER").val() != '' && !$("#onoffswitch_ldb").prop('checked') ) {
                    $.ajax({
                        url: "<?php echo ADMIN_SITE_URL;?>/modules/system/index.php?act=device_control&op=ctrlcmd",
                        type: "get",
                        timeout : 500,
                        dataType: "json",
                        data: { msg_name: 'FREQ_LDBEMER', msg_value : $("#FREQ_LDBEMER").val() },
                        success: function (t) {
                            if( t.status == '1' ) {
                                //alert("操作成功");
                            } else {
                ...
                //控制模式
                var c_model = $("#FREQ_LDBEMER").attr("data-curmod");
                //console.log(c_model);
                c_model = c_model & 6 ;
                //console.log(c_model);
