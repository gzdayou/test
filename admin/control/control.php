...
class SystemControl{
    ...
    protected final function getNav() {
        ...
        foreach ($_menu as $key => $value) {
            $top_nav .= '<li data-param="' . $key . '" data-url="'. C("admin_site_url") .'/?module='.$key.'"><a href="javascript:void(0);">' . $value['name'] . '</a></li>';
            ...
            foreach ($value['child'] as $ke => $val) {
                if (!empty($val['child'])) {
                    $left_nav .= '<dl><dt><a href="javascript:void(0);" class="first-'.$ke.'"><h3>' . $val['name'] . '</h3></a></dt>';
