...
class LoginControl extends SystemControl {
    ...
    public function loginOp(){
    	...
    	if( !empty($_POST) ) {
    		...
    		import('verify');
    		$verify = new verify;
    		$result = $verify->check($_POST['captcha']);
    		...
    		if ($error != '') {
    			...
    			if(is_array($admin_info) and !empty($admin_info)) {
    				...
    				$ret = array();
    				$ret['code'] = 1001;
    				$ret['msg'] = L('login_index_username_password_wrong');
    				die(json_encode($ret)) ;
    			}
    		}
			...
			$_SESSION["_csrf"] = md5($str);
			Tpl::output('_csrf', $_SESSION["_csrf"]);
    		Tpl::showpage('login','login_new2');
