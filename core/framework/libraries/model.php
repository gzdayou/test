...
//            if(isset($pkValue)) $data[$pk]   =  $pkValue;
     ...
     * @return boolean
     */
    public function updateRows($data='',$options=array()) {
        if(empty($data)) return false;
        // 分析表达式
        $options =  $this->parse_options($options);
        if(!isset($options['where'])) {
        ...
        return $this->db->updateRows($data,$options);
...
//                $whereStr .= ' )'.$operate;
                ...
                if(0===strpos($key,'_')) {
                    // 解析特殊条件表达式
                    $whereStr   .= $this->parseThinkWhere($key,$val).$operate;
                }else{
                    // 查询字段的安全过滤
                    if(!preg_match('/^[A-Z_\|\&\-.a-z0-9]+$/',trim($key))){
                        throw_exception($error);
...
//                    $whereStr .= ' ('.$key.' '.$val[1].') ';
                ...
                }elseif(preg_match('/SECOND/i',$val[0])){
                    $data = is_string($val[1])? explode(',',$val[1]):$val[1];
                    if($data[0] && $data[1]) {
